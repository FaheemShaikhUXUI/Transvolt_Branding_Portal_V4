...
import { Plus, UploadCloud, X, FileText, Image as ImageIcon, PenTool, ChevronDown, Check, Trash2, ArrowUpRight, Zap } from "lucide-react"
          ...
          <div className="flex items-center justify-between pt-1 gap-2">
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <Button
                ...
                variant="ghost"
                size="icon"
                className="h-7 w-7 text-muted-foreground hover:text-red-600 hover:bg-red-500/10 rounded-full cursor-pointer"
...
interface AddChargerBrandingModalProps {
  trigger?: React.ReactNode
  defaultCompany?: string
  defaultSiteLocation?: string
}

export function AddChargerBrandingModal({ trigger, defaultCompany, defaultSiteLocation }: AddChargerBrandingModalProps) {
  ...
  // Connected to Company Master: Extract registered companies connected to charger-branding (excluding any on hold)
  const existingCompanies = React.useMemo(() => {
    const masterCompanies = getUniqueCompanyNames("charger-branding")
    const list = new Set<string>(masterCompanies)
    assets.forEach((a) => {
      if (
        (a.category === "charger-branding" || (a as any).parentSlug === "charger-branding") &&
        a.companyName &&
        a.companyName.trim() &&
        !isCompanyOnHold(a.companyName.trim())
      ) {
  ...
  const existingSiteLocations = React.useMemo(() => {
    const masterLocations = getUniqueSiteLocations(companyName, "charger-branding")
    ...
    assets.forEach((a) => {
      if (
        ...
        !isSiteOnHold(companyName, a.titleName.trim())
      ) {
  ...
  const handleRemoveVariant = (id: string, e: React.MouseEvent) => {
    ...
    if (variants.length <= 1) {
      toast.error("At least one charger variant is required.")
      return
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newAssets = processedVariants.map((v) => ({
        ...
        category: "charger-branding",
        formats: v.formats,
        thumbnail: v.thumbnail,
        status: "active" as const,
      }))

      await addAssets("Charger Branding", newAssets, "charger-branding")

      toast.success(
        processedVariants.length > 1
          ? `${processedVariants.length} charger branding variant tiles added successfully!`
          : "Charger branding variant added successfully!"
      )
      setOpen(false)
      resetForm()
    } catch (err) {
      console.error(err)
      toast.error("Failed to add charger branding asset. Please try again.")
    } finally {
      ...
      <DialogContent className="sm:max-w-4xl p-6 sm:p-8 max-h-[92vh] overflow-y-auto">
        <DialogHeader className="text-left space-y-1">
          <DialogTitle className="text-2xl font-bold tracking-tight text-[#4472C4]">
            Add Charger Branding
          </DialogTitle>
          <DialogDescription className="text-sm text-muted-foreground">
            Configure EV charger branding specifications, specify variants, and upload preview images and production files.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6 mt-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5 text-left">
              <Label htmlFor="charger-category-company" className="text-xs font-bold uppercase tracking-wider text-foreground">
                Category as Company <span className="text-red-500">*</span>
              </Label>
              <AutocompleteInput
                id="charger-category-company"
            ...
            {/* Site Location */}
            <div className="space-y-1.5 text-left">
              <Label htmlFor="charger-site-location" className="text-xs font-bold uppercase tracking-wider text-foreground">
                Site Location <span className="text-red-500">*</span>
              </Label>
              <AutocompleteInput
                id="charger-site-location"
          ...
          <div className="rounded-xl border border-border/80 bg-card overflow-hidden shadow-sm">
                  ...
                  >
                    ...
                    {variants.length > 1 && (
                      <button
                        ...
                        className="p-0.5 rounded hover:bg-destructive/10 hover:text-destructive text-muted-foreground transition-colors cursor-pointer"
            ...
            <div className="p-4 sm:p-5 space-y-4">
              {/* Variant Name Input */}
              <div className="space-y-1.5 text-left">
                <Label htmlFor={`charger-variant-name-${activeVariant.id}`} className="text-xs font-semibold text-foreground">
                  Charger Variant Name <span className="text-red-500">*</span>
                </Label>
                <Input
                  id={`charger-variant-name-${activeVariant.id}`}
                  placeholder="e.g. 60kW DC Fast Charger - Canopy & Side Wrap, 22kW Dual Gun AC Station"
          ...
          <DialogFooter className="flex items-center justify-end gap-3 pt-2">
            ...
            >
              {isSubmitting ? "Saving Assets..." : "Save Charger Branding"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
